...
def deploy_task_edit(request, env_id, task_id):
    """
    :param request:
    :param env_id:
    :param task_id:
    ...
    if request.method == "GET":
        form = DeployTaskModelForm(task_object.env, instance=task_object)
        return render(request, 'web/deploy_task_form.html',
                      {'form': form, "env_id": env_id, 'env_object': task_object.env})
    form = DeployTaskModelForm(task_object.env, data=request.POST, instance=task_object)
    if form.is_valid():
        form.save(is_create=False)
        return redirect(reverse('deploy_task_list', kwargs={'env_id': env_id}))
    return render(request, 'web/deploy_task_form.html', {'form': form, "env_id": env_id, 'env_object': task_object.env})
